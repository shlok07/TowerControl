...
//           - irrTowerAvailable: if no reed pulse in 10 min, slot skips
//             tower immediately (checked every 5s by slot monitor).
// ======================================================================
...
static bool irrTowerAvailable(uint8_t t) {
  ...
  // [FIX-16] Also check fault and bypass flags explicitly
  if (towers[t].fault) return false;
  if (towers[t].bypassLine) return false;
  // [FIX-16] Activity check: no reed pulse in 10 minutes → tower is dead
  {
    unsigned long elapsed;
    if (towers[t].lastEdgeMs != 0)
      elapsed = millis() - towers[t].lastEdgeMs;
    else
      elapsed = millis() - towers[t].lastChangeMs;

    if (elapsed > DEAD_TOWER_TIMEOUT_MS) return false;
  }
  // not already done
