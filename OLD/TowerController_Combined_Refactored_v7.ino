...
//  [FIX-14] → Superseded by FIX-17.
//  [FIX-15] → Superseded by FIX-17.
//  [FIX-16] DEAD-TOWER DETECTION (emergency switch / physical stop):
//           - 10-minute hard timeout (DEAD_TOWER_TIMEOUT_MS = 600000).
//           - Fault backstop: if tower stays in learning mode for 10 min
//             with too few trays, declare fault.
//  [FIX-17] SIMPLE PER-TOWER IRRIGATION TOGGLES:
//           - Replaced batch/slot system with 8 independent toggles
//             (irrEnable0..irrEnable7), one per tower.
//           - Toggle ON = reset tray count, start irrigating.
//           - Toggle OFF = stop valve immediately.
//           - irrigationCycles (1-10) = full passes per tower.
//           - Tower auto-disables its toggle when complete.
// ======================================================================
...
// [FIX-16] Dead-tower timeout: if no reed pulse in this long, the tower
// is considered physically stopped (emergency switch, broken belt, etc.).
// Used by both fault backstop and irrigation slot skip.
static const unsigned long DEAD_TOWER_TIMEOUT_MS = 600000UL;  // 10 minutes
...
// [FIX-17] SIMPLE PER-TOWER IRRIGATION TOGGLES
// 8 cloud bools (irrEnable0..irrEnable7), one per tower.
// Toggle ON  = reset tray count, start irrigating.
// Toggle OFF = stop valve, ignore sensor.
// irrigationCycles (cloud int, 1-10) = how many full passes per tower.

static bool  irrTowerEnabled[NUM_TOWERS] = {false};
static int   irrigationCyclesInternal    = 1;
...
static inline bool anyIrrigationActive() {
  for (uint8_t i = 0; i < NUM_TOWERS; ++i)
    if (irrTowerEnabled[i]) return true;
  return false;
}

static inline bool isTowerIrrEnabled(uint8_t ch) {
  if (ch >= NUM_TOWERS) return true;   // non-tower ch always passes
  return irrTowerEnabled[ch];
}
...
void readReedAndUpdate(uint8_t i, unsigned long now) {
  ...
  // ---- FAULT DETECTION ---- [FIX-13 hardened] [FIX-16 dead-tower backstop]
  if (!t.contactorOn || t.fault || t.bypassLine || now <= t.inhibitUntilMs)
    return;

  // [FIX-16] Dead-tower backstop: if the tower has been "active" for 10
  // minutes without reaching the learning threshold, something is physically
  // wrong (e.g. emergency switch pressed). Fault it.
  if (t.traysSinceActive < LEARNING_TRAYS) {
    unsigned long activeFor = (t.lastEdgeMs == 0)
      ? (now - t.lastChangeMs) : (now - t.lastEdgeMs);

    if (activeFor > DEAD_TOWER_TIMEOUT_MS) {
      Serial.print(F("[FAULT] T")); Serial.print(i+1);
      Serial.print(F(" dead-tower backstop: active "));
      Serial.print(activeFor / 1000UL);
      Serial.print(F("s, only ")); Serial.print(t.traysSinceActive);
      Serial.print(F("/")); Serial.print(LEARNING_TRAYS);
      Serial.println(F(" trays. Physical stop suspected."));

      t.fault                   = true;
      t.faultCommandedOn        = t.contactorOn;
      t.faultExpectedIntervalMs = DEAD_TOWER_TIMEOUT_MS;
      t.faultTriggeredMs        = now - activeFor;
      t.faultEmailSent          = false;
      t.lastFaultEmailAttempt   = 0;
...
// [FIX-17] Per-tower irrigation helpers
...
// [FIX-17] Forward declarations (defined after fireValve in callbacks section)
static void setIrrCloudVar(uint8_t t, bool val);
static bool getIrrCloudVar(uint8_t t);

inline void fireValve(DigitalMechExpansion& exp, uint8_t ch, unsigned long now) {
  if (ch >= NUM_CH) return;
  const bool isTowerChannel = (ch < NUM_TOWERS);

  // [FIX-17] Simple check: is this tower's irrigation toggle on?
  if (isTowerChannel && !isTowerIrrEnabled(ch)) {
  ...
  if (isTowerChannel) {
    ...
    // Tower complete → auto-disable its toggle
    if (towerIrrigationComplete(ch)) {
      Serial.print(F("[IRR] T")); Serial.print(ch + 1);
      Serial.print(F(" complete (")); Serial.print(irrigationCyclesInternal);
      Serial.println(F(" cycle(s)). Auto-disabling."));

      irrTowerEnabled[ch] = false;
      setIrrCloudVar(ch, false);    // reflect to cloud
...
// [FIX-17] Helper: set one cloud variable by tower index
static void setIrrCloudVar(uint8_t t, bool val) {
  switch (t) {
    case 0: irrEnable0 = val; break;  case 1: irrEnable1 = val; break;
    case 2: irrEnable2 = val; break;  case 3: irrEnable3 = val; break;
    case 4: irrEnable4 = val; break;  case 5: irrEnable5 = val; break;
    case 6: irrEnable6 = val; break;  case 7: irrEnable7 = val; break;
  }
}

static bool getIrrCloudVar(uint8_t t) {
  switch (t) {
    case 0: return irrEnable0;  case 1: return irrEnable1;
    case 2: return irrEnable2;  case 3: return irrEnable3;
    case 4: return irrEnable4;  case 5: return irrEnable5;
    case 6: return irrEnable6;  case 7: return irrEnable7;
  }
  return false;
}

// [FIX-17] Single tower enable/disable
static void handleIrrToggle(uint8_t t) {
  bool nowEnabled = getIrrCloudVar(t);
  bool wasEnabled = irrTowerEnabled[t];
  irrTowerEnabled[t] = nowEnabled;

  DigitalMechExpansion exp1 = OptaController.getExpansion(IRR_EXP_IDX);

  if (!nowEnabled) {
    // Turning off → stop valve
    exp1.digitalWrite(t, LOW);
    outStateArr[t] = 0; offAtMs[t] = 0;
    firedThisAssert[t] = false;
    ignoreUntilMs[t] = millis() + IGNORE_MS;
    exp1.updateDigitalOutputs();
    Serial.print(F("[IRR] T")); Serial.print(t + 1);
    Serial.println(F(" irrigation OFF."));
    return;
  }

  if (!wasEnabled && nowEnabled) {
    // Turning on → reset and start
    unsigned long now = millis();
    towerIrrCount[t] = 0;
    exp1.digitalWrite(t, LOW);
    outStateArr[t] = 0; offAtMs[t] = 0;
    trigCountArr[t] = 0; lastTrigMsArr[t] = 0;
    prevTrigMsArr[t] = 0; deltaSecArr[t] = 0;
    PinStatus level = exp1.digitalRead(t);
    lastRaw[t] = level; lastStable[t] = level;
    lastChangeMsIrr[t] = now;
    firedThisAssert[t] = (level == ACTIVE_LEVEL);
    ignoreUntilMs[t] = 0;
    inputActiveStartMs[t] = 0; stuckSensorFault[t] = false;
    exp1.updateDigitalOutputs();

    Serial.print(F("[IRR] T")); Serial.print(t + 1);
    Serial.print(F(" irrigation ON, ")); Serial.print(irrigationCyclesInternal);
...
void onIrrEnable0Change() { handleIrrToggle(0); }
void onIrrEnable1Change() { handleIrrToggle(1); }
void onIrrEnable2Change() { handleIrrToggle(2); }
void onIrrEnable3Change() { handleIrrToggle(3); }
void onIrrEnable4Change() { handleIrrToggle(4); }
void onIrrEnable5Change() { handleIrrToggle(5); }
void onIrrEnable6Change() { handleIrrToggle(6); }
void onIrrEnable7Change() { handleIrrToggle(7); }
...
void onIrrigationCyclesChange() {
  ...
  // If a tower is now complete at new (lower) cycle count, auto-disable it
  for (uint8_t t = 0; t < NUM_TOWERS; ++t) {
    if (irrTowerEnabled[t] && towerIrrigationComplete(t)) {
      Serial.print(F("[IRR] T")); Serial.print(t + 1);
      Serial.println(F(" now complete. Auto-disabling."));
      irrTowerEnabled[t] = false;
      setIrrCloudVar(t, false);
    }
  }
}
...
void setup() {
  ...
  // [FIX-17] Per-tower irrigation toggles (start disabled)
  irrEnable0=false; irrEnable1=false; irrEnable2=false; irrEnable3=false;
  irrEnable4=false; irrEnable5=false; irrEnable6=false; irrEnable7=false;
  irrigationCycles=1;
  ...
  for (uint8_t t=0; t<NUM_TOWERS; t++) irrTowerEnabled[t]=getIrrCloudVar(t);
  irrigationCyclesInternal=irrigationCycles;
...
void loop() {
  ...
  if (di != lastDayIndex) {
    ...
    for (uint8_t i = 0; i < NUM_TOWERS; ++i) {
      towerTrayCount[i]=0;
      // [FIX-17] Only reset irrigation count if that tower's toggle is OFF
      if (!irrTowerEnabled[i]) towerIrrCount[i]=0;
    }
  ...
  for (uint8_t ch = 0; ch < NUM_CH; ++ch) {
    ...
    if (prev!=ACTIVE_LEVEL && raw==ACTIVE_LEVEL) {
      ...
      if (anyIrrigationActive() && isTowerIrrEnabled(ch))
  ...
  if (anyIrrigationActive()) {
    for (uint8_t ch = 0; ch < NUM_CH; ++ch) {
      if (!isTowerIrrEnabled(ch)) continue;
