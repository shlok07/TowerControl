...
static uint8_t        currentIrrBatch       = 0;

static inline bool channelInCurrentBatch(uint8_t ch) {
  if (ch >= NUM_TOWERS) return true;
  uint8_t batchStart = currentIrrBatch * TOWERS_PER_IRR_BATCH;
  return (ch >= batchStart && ch < batchStart + TOWERS_PER_IRR_BATCH);
}
...
bool           irrigationEnabledInternal = false;
...
static inline bool batchComplete(uint8_t batch) {
  uint8_t start = batch * TOWERS_PER_IRR_BATCH;
  uint8_t end   = start + TOWERS_PER_IRR_BATCH;
  for (uint8_t i = start; i < end && i < NUM_TOWERS; ++i)
    if (!towerIrrigationComplete(i)) return false;
  return true;
}
static inline bool allBatchesComplete() {
  for (uint8_t b = 0; b < NUM_IRR_BATCHES; ++b)
    if (!batchComplete(b)) return false;
  return true;
}
...
inline void fireValve(DigitalMechExpansion& exp, uint8_t ch, unsigned long now) {
  ...
  if (isTowerChannel && !channelInCurrentBatch(ch)) {
  ...
  if (isTowerChannel) {
    ...
    Serial.print(F(" (batch ")); Serial.print(currentIrrBatch);
    Serial.println(F(")"));

    if (towerIrrigationComplete(ch)) {
      Serial.print(F("[IRR] T")); Serial.print(ch + 1);
      Serial.println(F(" complete for this run."));
    }

    if (batchComplete(currentIrrBatch)) {
      if (currentIrrBatch + 1 < NUM_IRR_BATCHES) {
        currentIrrBatch++;
        Serial.print(F("[IRR] Batch ")); Serial.print(currentIrrBatch - 1);
        Serial.print(F(" complete → starting batch "));
        Serial.println(currentIrrBatch);

        uint8_t bStart = currentIrrBatch * TOWERS_PER_IRR_BATCH;
        uint8_t bEnd   = bStart + TOWERS_PER_IRR_BATCH;
        for (uint8_t c = bStart; c < bEnd && c < NUM_CH; ++c) {
          if (lastStable[c] == ACTIVE_LEVEL) {
            firedThisAssert[c] = true;
            Serial.print(F("[IRR] Ch ")); Serial.print(c);
            Serial.println(F(" sensor present at batch start – will count from next beep."));
          } else {
            firedThisAssert[c] = false;
          }
          ignoreUntilMs[c] = 0;
        }
      } else {
        Serial.println(F("[IRR] All batches complete. Disabling irrigation."));
...
void onIrrigationEnableChange() {
  ...
  bool nowEnabled = irrigationEnable;
  irrigationEnabledInternal = nowEnabled;

  DigitalMechExpansion exp1 = OptaController.getExpansion(IRR_EXP_IDX);

  if (!nowEnabled) { forceStopAllIrrigation(exp1); return; }

  if (!wasEnabled && nowEnabled) {
    unsigned long now = millis();
    forceStopAllIrrigation(exp1);

    currentIrrBatch = 0;

    for (uint8_t i = 0; i < NUM_TOWERS; ++i) towerIrrCount[i] = 0;
    for (uint8_t ch = 0; ch < NUM_CH; ++ch) {
      ...
      ignoreUntilMs[ch] = 0;
      offAtMs[ch] = 0; outStateArr[ch] = 0;
      inputActiveStartMs[ch] = 0; stuckSensorFault[ch] = false;
    }

    Serial.println(F("Irrigation enabled: batch 0 (towers 1-4), counting from next beep."));
  }
}
...
void setup() {
  ...
  for (uint8_t ch = 0; ch < NUM_CH; ++ch) {
    ...
    if (TRIGGER_ON_BOOT_HIGH && lastStable[ch]==ACTIVE_LEVEL && irrigationEnabledInternal) {
